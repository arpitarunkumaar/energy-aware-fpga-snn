...
if [[ $# -ne 2 ]]; then
    echo "usage: run_lif_io_saif.sh <label> <clock-period-ns>" >&2
    exit 2
fi
...
netlist="$sim_export/lif_unit_io_top_${label}_funcsim.v"
tb="$script_dir/tb/tb_lif_unit_io_power.sv"
...
"$vivado_bin/xelab" --log "$activity_dir/xelab.log" \
    ...
    --snapshot "tb_lif_unit_io_power_$label" tb_lif_unit_io_power glbl

export SN_SAIF_PATH="$saif"
"$vivado_bin/xsim" "tb_lif_unit_io_power_$label" \
    --tclbatch "$script_dir/xsim_lif_io_saif.tcl" \
