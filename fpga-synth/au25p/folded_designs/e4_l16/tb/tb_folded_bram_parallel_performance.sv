...
// Full-size cycle benchmark with ideal, gap-free spike and weight sources.
// Override NUM_ENGINES at elaboration to compare parallelism points.
module tb_folded_bram_parallel_performance #(
    ...
    always @(posedge clk) begin
        if (!rst) begin
            ...
            if (cycles - start_cycle > 64'd20000000)
                $fatal(1,
                       "timeout engines=%0d compute_state=%0d loader_state=%0d batch=%0d",
                       NUM_ENGINES, dut.compute_state, dut.loader_state,
                       dut.compute_batch);
        end
    end
    ...
    initial begin : weight_source
        integer beat;
        integer expected_neuron;

        expected_neuron = 0;
        forever begin
            wait (row_request_valid);
            if (row_request_neuron != expected_neuron)
                $fatal(1, "row request %0d arrived at position %0d",
                       row_request_neuron, expected_neuron);
    ...
    initial begin : stimulus
        ...
        $display("PERF lanes=%0d engines=%0d elapsed_cycles=%0d rows=%0d beats=%0d",
                 LANES, NUM_ENGINES, cycles-start_cycle, requests, beats);
        $finish;
    end
endmodule
