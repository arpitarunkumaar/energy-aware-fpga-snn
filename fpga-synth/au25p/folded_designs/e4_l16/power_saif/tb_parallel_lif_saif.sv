...
// The external row source is ideal and gap-free; its own power is not part of
// the synthesized core checkpoint.
module tb_parallel_lif_saif;
    ...
    localparam int ENGINES = `NUM_ENGINES;
    ...
    folded_bram_predictor_parallel dut (.*);
    ...
    always @(posedge clk) begin
        if (!rst) begin
            cycles <= cycles + 1;
            if ((cycles % 500000) == 0)
                $display("SAIF_PROGRESS engines=%0d cycles=%0d rows=%0d beats=%0d req=%0d ready=%0d",
                         ENGINES, cycles, request_count, beat_count,
    ...
    initial begin : stimulus
        ...
        for (word = 0; word < SPIKE_WORDS; word = word + 1) begin
            ...
            @(negedge clk);
            spike_valid = 1;
            spike_data = spike_frames[tstep][fold*LANES +: LANES];
        end
        @(negedge clk);
        spike_valid = 0;
        spike_data = 0;
        ...
        $display("SAIF_WORKLOAD_PASS engines=%0d cycles=%0d rows=%0d beats=%0d class=%0d",
                 ENGINES, cycles, request_count, beat_count, classification);
        repeat (5) @(negedge clk);
        $finish;
    end
endmodule
