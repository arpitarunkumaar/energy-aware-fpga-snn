...
if [[ $# -ne 3 ]]; then
    echo "usage: run_point.sh <label> <lanes> <clock-period-ns>" >&2
    exit 2
fi
...
netlist="$run_dir/folded_bram_predictor_funcsim.v"
...
"$vivado_bin/xvlog" --nolog --sv \
    ...
    "$script_dir/tb_folded_bram_saif.sv" >xvlog_tb.log 2>&1
"$vivado_bin/xelab" --nolog --relax --mt 8 --debug typical \
    -L unisims_ver -L unimacro_ver -L secureip \
    --snapshot "tb_saif_$label" tb_folded_bram_saif glbl \
    >xelab.log 2>&1
...
"$vivado_bin/vivado" -mode batch -nolog -nojournal \
    ...
    tb_folded_bram_saif/dut >power.log 2>&1

echo "SAIF_POINT_PASS label=$label lanes=$lanes period_ns=$period_ns"
